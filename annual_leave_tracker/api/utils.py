...
def portal_settings():
    doc = frappe.get_doc("Portal Setting", "Portal Settings")
    return doc
...
def send_login_mail(user, subject, template, add_args, now=None):
    ...
    args = {
        ...
        "login_url": "sme.payweaver.com" + "/login",
        "user_fullname": full_name,
    }

    args.update(add_args)

    sender = frappe.get_value("Email Account", "noreply@sme.payweaver.com", "email_id")
    if not sender:
        sender = (
...
def compare_dict(dict1 , dict2):
    """
    Dict1 is the recent value received from the UI
    Dict2 is what exist in the database
    """
    diff = {}
    for k1 in dict1.keys():
        for k2 in dict2.keys():
            if (k1 in k2) and dict1[k1]!=dict2[k1] and not (isinstance(dict2[k1], datetime.datetime) or isinstance(dict2[k1], datetime.date)):
                diff[k1] = {"new_value": dict1[k1], "old_value":dict2[k1]}
    return diff
